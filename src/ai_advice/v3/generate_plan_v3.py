...
import os, json, time, uuid, math, sys, argparse
...
PLAN_LATEST_PATH = data_dir / "plan_latest.json"  # fixed output for pipeline
...
def collect_topics_from_recommend(prefer_view: str = "default") -> Optional[str]:
    ...
    return (
        ...
        f"{topics_str}. You may gain a broader understand of the field by exploring these related research directions"
    )
...
def main():
    # 0) parse optional CLI args (goal/issue are mainly for logging or future metadata)
    parser = argparse.ArgumentParser()
    parser.add_argument("--goal", help="Optional override for learning goal", default=None)
    parser.add_argument("--issue", help="Optional GitHub issue id (for logging)", default=None)
    args = parser.parse_args()

    # 1) load inputs (lazy load files)
    ...
    # prefer CLI goal if provided, otherwise fall back to query from papers
    if args.goal:
        goal = args.goal
    else:
        goal = pick_goal_from_query(papers)

    # 2) build prompts + response_format
    messages = build_messages(
        goal=goal,
        papers=papers,
        study_level_hint=None,
        max_abstract_chars=800,  # keep context manageable
    )
    ...
    # 4) parse model output (must be a single JSON object)
    ...
    # 5) validate schema + runtime constraints
    ...
    # 7) write artifact with trace id (for debugging / history)
    artifact_path = CONFIG["ARTIFACT_DIR"] / f"plan_{trace_id}.json"
    artifact_path.write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding="utf-8")

    # 7.1) also write a fixed "latest" file for pipeline to pick up
    ...
    # 8) collect usage + cost (best-effort)
    ...
    log_obj = {
        ...
        "latest_path": str(PLAN_LATEST_PATH),
    ...
    # 10) print a short summary to console
    print("\n== Run Summary ==")
