...
def run_hdbscan(X: np.ndarray, min_cluster_size: int, min_samples: int | None, method: str, eps: float):
    ...
    kwargs = dict(min_cluster_size=min_cluster_size, metric="euclidean", cluster_selection_method=method, prediction_data=True)
            ...
            for method in METHODS:
                for eps in EPS_RANGE:
                    try:
                        ...
                        if sc > best_score:
                            best_score = sc
                            best = dict(min_cluster_size=mcs, min_samples=ms, method=method, eps=eps, metric=metric, metrics=metr)
                            sil_txt = "NA" if metr["silhouette"] is None else f"{metr['silhouette']:.4f}"
                            print(f"  best so far: score={best_score:.4f} sil={sil_txt} k={metr['k']} coverage={metr['coverage']:.2f} noise_rate={metr['noise_rate']:.2f} params={{'min_cluster_size':{mcs},'min_samples':{ms},'method':'{method}','eps':{eps},'metric':'{metric}'}}")
...
def main():
    ...
    print(f"[i] Params used: {{'min_cluster_size': {params['min_cluster_size']}, 'min_samples': {params['min_samples']}, 'method': '{params['method']}', 'eps': {params['eps']}, 'metric': '{final_metric}'}}")

    print("\n[5] Clustering on full data...")
    clusterer, labels = run_hdbscan(Z, params["min_cluster_size"], params["min_samples"], params["method"], params["eps"])
